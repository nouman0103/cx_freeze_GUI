...
def help6c():
    tmb.showinfo("InitScript","Write the path to InitScript( __init__.py),if any")
def help7c():
    tmb.showinfo("Base","Select the base")
def help8c():
...
def logp(text,color='green',wait=0.5,load=1,cl=False):
    global colo
    colo = "blue"
    if load == 1:probar.step(4)
...
def write_setup():
    ...
    spy.write(r'initsc = "'+pinit+'"');nl()
    spy.write(r'base = "'+pbase+'"');nl()
    spy.write('setup(\n    ')
    spy.write('name = name,\n    version = vers,\n    description=descr,\n    options = {"build_exe":option},\n    ')
    spy.write('executables=[Executable(file,initScript=initsc,base=base')
...
def build():
    ...
    if len(pinit) != 0:
        logp("Verifying InitScript path")
        if os.path.exists(pinit) == False:logp("FileNotFoundError: InitScript path not found","red");return 0
        else:
            if pinit.lower().endswith((".py",".pyw")) == True:pass
            else:logp("Invalid InitScript file","red");return 0
...
tkbg(root)
#fonts
high = tkinter.font.Font(root,family="Courier New",size=20)
high1 = tkinter.font.Font(root,family="Courier New",size=15)
...
#heading
sep = Label(root,text="______________________",font=high)
sep.place(x=97,y=14)
titl = Label(root,text="Create Executable File",font=high)
titl.place(x=97,y=10)
tkbg(sep);tkbg(titl)
#script
...
#icon
icon = Label(root,text="Icon:",font=low)
icon.place(x=12,y=220)
tkbg(icon)
icoent = Entry(root,width = 31,font=low1,bd=2)
icoent.place(x=150,y=224)
icobut = Button(root,text="Select",font=low1,command=butico)
icobut.place(x=440,y=220)
#initscript
initscr = Label(root,text="InitScript:",font=low)
initscr.place(x=12,y=260)
tkbg(initscr)
iscent = Entry(root,width = 31,font=low1,bd=2)
iscent.place(x=150,y=264)
iscbut = Button(root,text="Select",font=low1,command=butisc)
iscbut.place(x=440,y=260)
#base
basescr = Label(root,text="Base:",font=low)
basescr.place(x=12,y=300)
tkbg(basescr)
...
comlu.place(x=12,y=460)
#Build Button
buildb = Button(root,text="Build",font=high1,command=build)
